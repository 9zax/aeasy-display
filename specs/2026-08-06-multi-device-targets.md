...
2. **`adb reverse` rules live on the device, so the device-side port never has to change.** `adb help` describes `reverse` as listing and removing "reverse socket connections **from device**" — the table is in the phone's `adbd`. So `adb -s S2 reverse tcp:7355 tcp:7365` lets device 2 keep dialling `127.0.0.1:7355` while landing on the Mac's 7365. The Android app hardcodes the host at `Pane.kt:197` / `ControlClient.kt:57` and the port at `MainActivity.kt:27`, and needs to know nothing. `iproxy` takes an arbitrary local port with `-u UDID` (verified against the installed build), and the iOS app is the *listener* on its own 7355 (`StreamListener.swift:24`) and never sends a handshake at all — `grep AEZ1 ios/` is empty, so it lands on the server's 300 ms legacy path (`AEasyServer.swift:644-653`) exactly as it does today. **No wire-protocol change and no client change of any kind** — no APK rebuild, no Xcode round trip — and the iOS client's one-connection ceiling stops being a limitation because each iOS device gets its own relay. (That ceiling was `newConnectionLimit = 1` in `StreamListener.swift` when this spec was drafted; it was removed independently during implementation, for an unrelated reason — the limit decrements per accepted connection and at 0 the listener stops accepting forever, so a Mac restart could never reconnect. Either way this spec does not depend on it.)
...
- **FR-29** — The window gains a per-device **resolution lock** writing `PANEL` (FR-21) and an **input control** toggle writing `INPUT` (FR-23). *As built this is a checkbox, not the radio group the draft called for:* a radio group presumes every device is on screen at once, and FR-26 gives each device its own window. The checkbox shells `aeasy device input <slot>`, whose clear-then-set is what actually enforces exclusivity; unticking is refused with a note to tick another device instead, since something must hold input. The radio group survives in the tray (FR-32), where all devices *are* listed together.
...
- **FR-33** — Two tray-side corrections: a new `shellOut(_:) -> String` helper, because `shell()` sends stdout to `FileHandle.nullDevice` (`:11`) and so cannot read a device list; and `serverRunning()` (`:19`) becomes a per-slot count, since one `pgrep -qf 'aeasy-serve[r]'` reports healthy while two of three devices are dead. `cablePlugged()` (`:20-22`) is replaced by the enumeration.
  *The draft also required `start()`/`stop()` (`:118-119`) to take the selected slot; as built they stay global.* Their labels say "Start / Restart" and "Stop", and once `aeasy restart` loops the registry and `aeasy stop` is meant to stop everything, global is the correct reading — a Stop item that silently stopped one of three devices would be the surprising one. Per-device control is the Devices submenu, where removing a device stops exactly that device.
